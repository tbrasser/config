...
# destination: /config/packages/sensor.yaml
# src vars: /config/lovelace_rooms.yaml
#           /config/lovelace_views.yaml
...
sensor:
  - platform: template
    sensors:
      count_lights_on:
        value_template: >
          {{ expand('light.dummy') | selectattr('state','eq','on') | list | count }}
      count_devices_due:
        value_template: >
          something here
      count_devices_on:
        value_template: >
          something here
      count_devices_total:
        value_template: {{ state.sensor.count_devices_due + state.sensor.count_devices_on }}