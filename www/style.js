...
function findGridElement() {
  ...
  current = current.shadowRoot.querySelector('hui-vertical-stack-card')
  if (!current || !current.shadowRoot) return null
  
  current = current.shadowRoot.querySelector('#root > hui-conditional-card > mod-card')
  if (!current || !current.shadowRoot) return null
  
  current = current.shadowRoot.querySelector('ha-card > hui-vertical-stack-card')
  if (!current || !current.shadowRoot) return null
  
  // Try both possible layout-card locations
